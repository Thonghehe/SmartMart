...
public class MHdangky extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mhdangky);
        ...
        btnDangKy.setOnClickListener(v -> {
            ...
            if (isValidEmail(email) && isValidPassword(password) && password.equals(confirmPassword)) {
                ...
                if (!isValidEmail(email)) {
                    Toast.makeText(MHdangky.this, "Email không hợp lệ!", Toast.LENGTH_SHORT).show();
                } else if (!isValidPassword(password)) {
                    Toast.makeText(MHdangky.this, "Mật khẩu phải có ít nhất 6 kí tự!", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(MHdangky.this, "Mật khẩu không khớp!", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        auth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        Intent intent = new Intent(this, MHdangnhap.class);
                        startActivity(intent);
                        finish();
                    } else {
                        Log.w(TAG, "createUserWithEmail:failure", task.getException());
                        Toast.makeText(MHdangky.this, "Đăng ký thất bại!", Toast.LENGTH_SHORT).show();
                    }
                });
    }
}